...
function resolveOpts(opts = {}) {
  return {
    sizeThreshold:
      opts.sizeThreshold ?? Number(process.env.IMAGE_SIZE_THRESHOLD ?? DEFAULT_SIZE_THRESHOLD),
    maxDimension:
      opts.maxDimension ?? Number(process.env.IMAGE_MAX_DIMENSION ?? DEFAULT_MAX_DIMENSION),
    jpegQuality:
      opts.jpegQuality ?? Number(process.env.IMAGE_JPEG_QUALITY ?? DEFAULT_JPEG_QUALITY),
  };
}
...
export async function compressFile(filepath, opts = {}) {
  ...
  if (out.length >= stat.size) {
    return { status: 'no-gain', filepath, beforeSize: stat.size, afterSize: out.length };
  }

  await fs.writeFile(filepath, out);
  return { status: 'compressed', filepath, beforeSize: stat.size, afterSize: out.length };
}
...
if (isCli) {
  ...
  for (const r of results) {
    if (r.status === 'compressed') {
      console.log(`  ${r.filepath}: ${fmt(r.beforeSize)} → ${fmt(r.afterSize)}`);
      changed++;
    } else if (r.status === 'no-gain') {
      console.log(`  skip ${r.filepath} (${fmt(r.beforeSize)} → ${fmt(r.afterSize)}, no gain)`);
    }
  }
