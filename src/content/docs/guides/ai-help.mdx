...
File naming: "name.mdx" (slug based on the person's name; testimonials are sorted by the `order` frontmatter field)
Frontmatter:
  quote: string (required)
  name: string (required)
  role: string (required)
  order: number (required) — display order (lower = shown first). Ties fall back to the slug.
...
Locale-agnostic — one file per person; roles translate inline via roleEn/roleDe.
File naming: "name.mdx" (slug based on the person's name). Used by the Faces of BEARS grid AND by projects' Meet the Team feature (via projects.person reference).
Frontmatter:
  name: string (required)
  roleEn: string (required) — English role
  roleDe: string (required) — German role
  coverImage: string (required) — path under src/assets/people/<slug>/, written as "<slug>/<filename>", valid: .jpg .jpeg .png .webp .svg
...
Frontmatter fields:
  ...
  socialLinks: array — each: { platform, iconFile, url, hoverColor? } — iconFile must be an .svg filename under src/assets/social-icons/ (upload a new SVG via the Social links editor to add a new platform; icons render as silhouettes via CSS mask)
  ...
  mediaCategories: array — each: { id, label } — controls which categories appear on the Media page and in what order
    Available ids: all (combined view), about-us, events, people, hero, projects, testimonials, what-is-bears
...
Page text folder structure (inside en/ and de/):
  ...
  footer/         — footer (footer-address — multiline address block)
...
Outlier singletons at the locale root (one .mdx file each, not in a subfolder):
  ...
  social.mdx            — site-wide social media links (socialLinks array; iconFile is an .svg uploaded to src/assets/social-icons/)
...
Special behaviors:
  ...
  footer/footer-address.mdx uses the address field (multi-line text), not items, so the address block can be edited as a free-form block.
...
All images must be local files in src/assets/ subdirectories. Events, projects, sponsors, testimonials, and people use a per-slug asset layout — each entry gets its own subfolder whose name matches the entry's filename (without extension):
  ...
  src/assets/people/<slug>/         — per-person portrait (used by Faces of BEARS grid + Meet the Team)
...
- Sort order is controlled differently per collection: events/projects/instagram sort by the `date` frontmatter field; sponsors/hero-slides/testimonials sort by the `order` frontmatter field; people sort by the `order` field within the Faces of BEARS grid (only when showInFaces is true). Lower = shown first; ties fall back to the slug. No filename prefixes are used for ordering — slugs should be human-readable (e.g. `jane-doe.mdx`, not `01-jane-doe.mdx`).
...
- Footer address is configured in footer/footer-address.mdx via the multi-line `address` field (each line in the YAML renders as its own line on the page).
