...
File naming: identifier (by convention matches the referenced person's slug). Each testimonial references one person from the `people` collection and supplies the quote + display order for the landing-page Testimonials carousel.
Frontmatter:
  identifier: string (required) — used for the filename; conventionally matches the person's slug
  person: reference('people') (required) — slug of the person this testimonial belongs to
  order: number (default 0) — lower = shown first in the carousel; ties fall back to identifier
