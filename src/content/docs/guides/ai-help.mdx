...
- Sort order is controlled differently per collection: events/projects/instagram sort by the `date` frontmatter field; sponsors/hero-slides/testimonials sort by the `order` frontmatter field; faces-of-bears sorts alphabetically by slug (use numeric prefixes like 01-, 02- in the filename to control order).
