...
TESTIMONIALS (src/content/testimonials/list.yaml)
Single-entry data collection backing the landing-page Testimonials carousel. Managed in Keystatic as a drag-reorderable list (a singleton with fields.array); array order in the file IS the display order — no sort key, no per-item filenames.
Schema:
