...
│   ├── media-categories.mdx  — Media page categories
...
| `socialLinks` | array | Social media links, each with `platform`, `iconFile` (uploaded SVG), `url`, optional `hoverColor` |
...
A heading plus a free-form multi-line text block, rendered line-for-line. Used for the footer address.
...
**Where it appears:** The footer address block. Each line in the `address` field renders as a separate line.

**Fields used:** `title`, `address`

**Files:** `footer/footer-address`
...
**Fields used:** `title`, `subtitle` (optional label above heading), `description` (optional intro text), `room`, `schedule`, `mapLat`, `mapLng`
...
Each entry specifies a `platform` label, an `iconFile` (an SVG uploaded through the CMS), and a `url`. You can optionally set a custom `hoverColor` per entry.
...
**Adding a new social platform:** In the **Social links** singleton, click **Add**, pick an **Icon (SVG)** from your computer (the file is uploaded to `src/assets/social-icons/` and the filename is saved into `iconFile`), set a URL, and save. Nothing else to change — the Footer and Contact page pick it up automatically. The Instagram homepage CTA is wired by URL (`instagram.com`), so the Instagram entry stays discoverable regardless of its label.

**`iconFile`:** Required. An SVG file uploaded into `src/assets/social-icons/`. Icons render as a **silhouette** via CSS mask — the fill colour inside the SVG is ignored, which is what lets the hover animation swap between dark, white, and `hoverColor`. For best results, use a single-colour 24&times;24 SVG (the same shape you'd find on [simpleicons.org](https://simpleicons.org)). An AI can also generate one for you — ask for a single-path 24&times;24 silhouette SVG of the platform's logo.

**`platform`:** A free-form label (e.g. `"instagram"`). Used as the accessible label and as the item title in the CMS list. You can set it to any readable name.

**`hoverColor`:** Optional. Sets a custom background colour when hovering over the icon. If omitted, the default accent colour is used. Use standard hex colour codes (e.g., `"#0A66C2"` for LinkedIn blue).
...
| `people` | Person portraits — Faces of BEARS + Meet the Team (`src/assets/people/`) |
...
**File:** `media-categories.mdx` (at the locale root). Edited in the CMS via the **Media categories** singleton.
