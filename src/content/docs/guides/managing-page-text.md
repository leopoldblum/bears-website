...
├── media/         — Media page
├── footer/        — Footer (navigation, social links, address, legal)
├── site/          — Site-wide metadata (site title, default description)
├── 404/           — Not Found page
...
Defines the social media icons and URLs shown in the footer. Each entry specifies a `platform` name (which determines the icon) and a `url`. You can optionally set a custom `hoverColor` per platform.
...
**Where it appears:** The social media icon row in the footer.

**Supported platforms:** `instagram`, `linkedin`, `youtube` (icons are mapped by platform name in code).
