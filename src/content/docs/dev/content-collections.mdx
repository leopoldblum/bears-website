...
| `people` | `src/content/people/` | No (roles inline per locale) | name, roleEn, roleDe, coverImage (optional — falls back to `defaultFaceImage`), showInFaces, order |
...
| `hero-slides` | `src/content/hero-slides/` | No | order, alt, shownText, media (conditional: `{discriminant: image\|video, value}`) |
