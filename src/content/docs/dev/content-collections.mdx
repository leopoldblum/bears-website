...
| `testimonials` | `src/content/testimonials/{en,de}/` | Yes | quote, name, role, order, coverImage |
| `people` | `src/content/people/` | No (roles inline in `roleEn`/`roleDe`) | name, roleEn, roleDe, coverImage, showInFaces, order |
...
This is the project's first cross-collection reference — follow the same pattern (`reference()` + `fields.relationship()`) for any future ones.
...
| `socialLinks` | array? | Platform label + required iconFile (uploaded SVG) + URL + optional hoverColor |
