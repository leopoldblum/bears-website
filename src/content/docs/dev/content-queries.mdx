...
| Function | Returns | Notes |
|----------|---------|-------|
| `getTestimonials(locale?)` | `CollectionEntry<'people'>[]` | Reads the single-entry `testimonials` data collection (`list.yaml`), resolves each item's `person` reference, preserves array order, projects locale-correct `role` (from the person) and `quote` (from the item) onto `data`; skips + warns on unresolved refs |
