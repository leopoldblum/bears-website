...
| Function | Returns | Notes |
|----------|---------|-------|
| `getTestimonials(locale?)` | `CollectionEntry<'people'>[]` | Reads the `testimonials` collection, resolves each `person` reference, sorts by `order`, projects locale-correct `role` (from the person) and `quote` (from the testimonial) onto `data`; skips + warns on unresolved refs |
