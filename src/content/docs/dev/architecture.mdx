...
Localized collections (`events`, `projects`, `page-text`) use `en/` and `de/` subfolders. Language-neutral collections (`sponsors`, `instagram`, `hero-slides`, `docs`, `people`, `social-platforms`) stay flat — the `people` collection carries both translations inline (`roleEn`/`roleDe`, and `quoteEn`/`quoteDe` for the testimonials surface).
